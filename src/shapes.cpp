...
public:
    ...
    double Distance(const Point& pt) const {
        return sqrt( pow(coords_[0] - pt.GetX(), 2) + pow(coords_[1] - pt.GetY(), 2) );
    }
...
public:
    ...
    void AddEye(unique_ptr<Shape> s) {
        eyes_.push_back(move(s));
    }

    void SetMouth(unique_ptr<Shape> s) {
        mouth_ = move(s);
    }
...
private:
    ...
    vector<unique_ptr<Shape>> eyes_;
    unique_ptr<Shape> mouth_;
};
